...
function sendLimitOrderEmail(order) {
  console.log(order)
  var orderMsg = "";
  var subjectWord = order.ordStatus === "Filled" ? "filled" : order.ordStatus;
  var subject = order.orderType + " " + order.side + " order " + subjectWord + " for " + order.symbol 
  orderMsg += subject + ".\n\n";
...
function sendMarketOrderEmail(order) {
  var orderMsg = "";
  var subject = order.orderType + " " + order.side + " order placed for " + order.symbol;
  orderMsg += subject + ".\n\n";
  orderMsg += "Order Quantity: " + order.orderQty + "\n\n";
  orderMsg += "Order Price: " + order.price + "\n\n";
  orderMsg += "Order Status: " + order.ordStatus + ".";
...
function sendErrorEmail(e) {
  var subject = "Phemex Bot: An Error has Occurred."
  sendEmail(e, subject);
}
...
function sendTextErrorEmail(err) {
  var subject = "Phemex/Binance Bot: A Text Message Related Error has Occurred."
